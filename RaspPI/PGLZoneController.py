...
class PGLZoneController:
    ...
    def __init__(self, num_zones: int, devices_model: PGLModel = None):
        ...
        # And initialise ledstates to off
        if devices_model:
            ...
            for index, device in enumerate(devices_model.actuators_list):
                ...
                self.led_states[device.id_] = "OFF"
        ...
        self.journey = PGLJourney(
            self.zone_count - 1, self.server_api.add_event_to_queue)
    ...
    def set_device_led_states(self, zone_to_turn_on: tuple[int, int]):
        led_ids: list[str] = self.get_device_ids_from_zone_ids(
            list(zone_to_turn_on), types=["led", "led"])
