...
class PGLZigbee2mqttMessage:
    ...
    # can be observed in the parse() method, where instances of PGLZigbee2mqttMessage are created
    ...
    topic: str
    type_: PGLZigbee2mqttMessageType
    data: Any = None
    event: Any = None
    message: Any = None
    meta: Any = None
    ...
    @classmethod
    def parse(cls, topic: str, message: str) -> PGLZigbee2mqttMessage:
        ...
        # declared. In this case, this method returns an instance of PGLZigbee2mqttMessage based on
        ...
        if topic == "zigbee2mqtt/bridge/state":
            instance = cls(type_=PGLZigbee2mqttMessageType.BRIDGE_STATE,
                           topic=topic,
                           state=message)
        elif topic in ["zigbee2mqtt/bridge/event", "zigbee2mqtt/bridge/logging"]:
            type_ = {"zigbee2mqtt/bridge/event": PGLZigbee2mqttMessageType.BRIDGE_EVENT,
                     "zigbee2mqtt/bridge/log": PGLZigbee2mqttMessageType.BRIDGE_LOG}.get(topic)
...
class PGLZigbee2mqttClient:
    ...
    def __init__(self,
                 host: str,
                 on_message_clbk: Callable[[Optional[PGLZigbee2mqttMessage]], None],
                 port: int = 1883,
    ...
    def __worker(self) -> None:
        ...
        while not self.__stop_worker.is_set():
            try:
                ...
                if message:
                    self.__on_message_clbk(PGLZigbee2mqttMessage.parse(message.topic,
