...
class PGLEvemanEvent:
    ...
    @classmethod
    def from_json(cls, event: Union[str, Dict[str, Any]]) -> PGLEvemanEvent:
        """ Instantiate a PGLEvent from a JSON string.

        Args:
            event (Union[str, Dict[str, Any]]): the JSON object as a string or as a dict.

        Returns:
            PGLEvent: object instantiated fro mthe JSON object.
...
class PGLEvemanModel:
    ...
    def get_events(self, device_id: str = None) -> List[PGLEvemanEvent]:
        ...
        for (device_id, device_type, measurement, timestamp) in cursor:
            ...
            events.append(PGLEvemanEvent(device_id=device_id,
    ...
    def store(self, event: PGLEvemanEvent) -> None:
        if not self.__mysql_connection:
            raise RuntimeError(f"Not connected to database {self.__database}.")
...
class PGLEvemanController:
    ...
    def __init__(self, mqtt_host: str, model: PGLEvemanModel, mqtt_port: int = 1883) -> None:
        ...
        while not self.__stop_worker.is_set():
            try:
                ...
                if message.topic == self.STORE_EVENTS_TOPIC:
                    try:
                        event = PGLEvemanEvent.from_json(message.payload.decode("utf-8"))
...
def main():
    ...
    model = PGLEvemanModel(host="localhost",
                            database="PGL",
                            user="PGL",
                            password="PGL")
    
    # The controller will connect to a MQTT broker running in the same machine.
    controller = PGLEvemanController(model=model,
                                      mqtt_host="localhost")
